...
EXTERN int need_atr;      /* Требуемые атрибуты */
/*
 * Графические символы
 */
#ifdef UTF8
#define G_START 0xc0
#define G_GH    0xc0  /* Гориз линия */
#define G_GF    0xc1  /* Заполненный квадрат */
#define G_GV    0xc2  /* Верт. линия */
#define G_LD    0xc3  /*                                */
#define G_MD    0xc4  /*     LU MU  RU                  */
#define G_RD    0xc5  /*     LM CC  RM                  */
#define G_LM    0xc6  /*     LD MD  RD                  */
#define G_CC    0xc7  /*     - GH                       */
#define G_RM    0xc8  /*     | GV                       */
#define G_LU    0xc9  /*                                */
#define G_MU    0xca  /*                                */
#define G_RU    0xcb  /*                                */
#define G_KU    0xcc  /* Стрелка вверх                  */
#define G_KD    0xcd  /* Стрелка вниз                   */
#define G_KL    0xce  /* Стрелка влево                  */
#define G_KR    0xcf  /* Стрелка вправо                 */
#define G_END   0xcf
#else //UTF8
#define G_START 0241
...
EXTERN struct fsd *openfsds[MAXFILES];
EXTERN char *openfnames[MAXFILES],
      openwrite[MAXFILES],       /* 1 - файл можно записывать */
      movebak[MAXFILES];         /* 1 - файл уже двигали в .bak */
EXTERN int  nlines[MAXFILES];          /* Число непустых строк в файле */
...
EXTERN struct workspace *curwksp, *pickwksp;
EXTERN int curfile;
...
EXTERN struct viewport *portlist[MAXPORTLIST+1],
...
EXTERN struct savebuf *pickbuf, *deletebuf;

/*
 * Управляющие символы
 */
//#define ISACMD(c)        (0300 == (c&0300))  //UTF8
#define ISACMD(c)        (0300 == (c&0300) && c != 0xd0 && c != 0xd1 )  //UTF8
#define CC                0300     /* bvg UTF8 Начало упр. кодов внутри red */
...
//#define CCPLSRCH           020+CC     /* plus search    D0  UTF8    */
//#define CCMISRCH           021+CC     /* minus search   D1  UTF8    */
#define CCPLSRCH           064+CC     /* plus search      F3  UTF8  */
#define CCMISRCH           065+CC     /* minus search     F4  UTF8  */
...
EXTERN int cursorline;         /* physical screen position of */
EXTERN int cursorcol;          /*  cursor from (0,0)=ulhc of text in port */
 ...
 */
EXTERN int paraml;
EXTERN char *paramv;EXTERN int paramtype;
EXTERN int paramc0, paramr0, paramc1, paramr1;
...
EXTERN int LINEL,NLINES; /* SIZE OF THE SCREEN  - VT.C */
EXTERN int eolflag;
...
#ifndef DEMOSES
EXTERN char *(*agoto)();  /* for termcap definitions */
extern int (*atcread)();
EXTERN int slowsw;   /* is 1 if slow speed terminal */
...
#endif
#ifdef RED_ALT     /* bvg */
EXTERN int Rcy_alt;       /* bvg */
EXTERN int Rcy_ALT;       /* bvg */
#endif             /* bvg */

EXTERN int on_scroll; /* Флаг, разрешающий roll */
EXTERN int fix_screen; /* Флаг запрещает автоматическую прокрутку экрана */
EXTERN int on_autoi;   /* Флаг включехия авто-сдвига */
EXTERN int autoindent; /* Auto-indent */
#else
EXTERN int rawf, lcasef, latf, flgtabs, speccase; /* terminal regim flags */
#endif
EXTERN int new_info; /* Признак - перерисовать info */
EXTERN int need_box; /* Признак - перерисовать рамку */
...
EXTERN int zoomflag;            /* Признак "zoom" для повтора по умолчанию */
/*
 * Переменные для режима выборки текста
 * флаг, абс. координаты, номер файла
 */
EXTERN int sel_flag;
EXTERN int sel_lin, sel_col, sel_file;
EXTERN struct workspace *sel_wspace;
extern int defedit;

...
register lr1;
{
	...
	if ((i = cursorcol + curwksp->ulhccno ) >= (lcline - 2)) excline(i+2);
	if (i >= ncline-1){
...
repop:
	...
	/* Строки у нас нет? Дай! */
	old = getlineFlerov( (i=ABS_LIN(cursorline)) );
	/* исключение символа */
...
/* --- */
    ...
    sk = searchkey;
    while (*sk++) lkey++;
    getlineFlerov (ln = lin + curwksp->ulhclno);
    /* putline(0);      */
    at = cline + col + curwksp->ulhccno + tUTF8(col+curwksp->ulhccno);
    FOREVER
	{
	at += delta;
